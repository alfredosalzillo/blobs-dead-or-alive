...
export const generateRandomBlobs = (size = 9) =>
  Array(size)
    .fill(0)
...
const calculatePoints = (game: GameControllerState) =>
  Math.round((MAX_ROUND_POINTS * (game.time - game.elapsed)) / game.time);
const calculateTimeBonus = (game: GameControllerState) =>
  MAX_BONUS_TIME / 2 + MAX_BONUS_TIME / 2 ** (game.round - 1);
...
const rush: GameControllerStrategy = (state, action) => {
  ...
  switch (action.type) {
    ...
    case "update-elapsed":
      ...
      return updateElapsed(
        Math.min(state.elapsed + action.payload, state.time),
      );
    default:
      return state;
  }
